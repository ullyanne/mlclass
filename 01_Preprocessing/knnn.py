...
# Use the mask to create two new DataFrames
feature_cols_i = ['Pregnancies', 'Glucose', 'BloodPressure', 'SkinThickness', 
...
neigh_i = KNeighborsClassifier(n_neighbors=7)
neigh_i.fit(Xi, yi)

test = data_test.drop('Insulin', axis=1)
df_with_nan = df_with_nan.drop('Insulin', axis=1)

y_pred_i = neigh_i.predict(df_with_nan)
df_with_nan['Insulin'] = y_pred_i

df_merge_col = pd.concat([df_with_nan, df_without_nan], axis=0)
